...
Analysis of 5,000 e-commerce visitors found 826 purchases generating $87,975 in revenue, equivalent to a 16.5% visitor-to-purchase conversion rate. Email showed the strongest conversion performance, while the largest funnel drop occurred between initial visit and add-to-cart.

- SQL: CTEs, CASE WHEN, aggregation, DISTINCT, date filtering, views, conversion calculations
- Excel: data import, KPI reporting, bar charts
- Analysis: funnel analysis, channel attribution, revenue analysis, business recommendations
...
The four CTEs used were: 
  - funnel_revenue - extracted total visitors, buyers, orders and revenue and other revenue metrics including revenue per order, buyer and visitor.
  - source_funnel - extracted information about the types of ads being run, including their popularity and conversion rates
...
Amount of users per purchasing stage. A total of 5,000 potential customers entered the website between Dec 2025 and Feb 2026. 826 orders were made by 826 buyers, indicating one order per buyer. 
...
Overall conversion rate from each individual ad campaign. The number indicates the percentage of potential customers who became paying customers. Email had the highest overall conversion rate.
...
• Audit Ad Spend against average order value (AOV) which was ~ $106.
  • Action: Social generates high traffic volume but has a lower conversion rate than email. Further analysis incorporating channel-level ad spend and            customer acquisition cost (CAC) would be required before reallocating budget. Provisional action given the data is to reduce budget around social media.

## Limitations 

- There is no outlined customer acquisition cost (CAC) to compare the success of each ad campaign being ran, i.e. how much profit is being made per customer brought in by an individual ad campaing.
...
- Conversion analysis uses the earliest page_view, add_to_cart and purchases recorded per user within the specified period. Potentially, this could lead to strange user journey data - i.e. Jan 2: purchase, Jan 10: page view and Jan 12: add to cart. The next analysis should use session/order identifiers to provide more realistic journeys
